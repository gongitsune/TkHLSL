...
- **BCL ギャップとポリフィル**: netstandard2.0 には `IsExternalInit`, `required` 属性, `NotNullWhen` 等が存在しない。`PolySharp`（ランタイム依存なしのソースジェネレータ型ポリフィル）の採用を検討
  - → **決定（Phase 3 で対応）**: `TkHLSL/TkHLSL.csproj` に `PolySharp` を両 TFM 共通で追加（`PrivateAssets="all"`、ビルド時のみ有効なポリフィルでランタイム依存なし）。`Ir/*`（`TypeInfo`/`GlobalVariable`/`Function`/`EntryPoint`）で `record` 型（`IsExternalInit` が必要）を使うため、Phase 3 で実際にギャップに到達し採用を確定した
...
- **Phase 5 公開モデル構築のアロケーション方針**: `HlslParser.Parse` が `Module`+`ModuleInfo` から `HlslCompilationResult` を組み立てる際、カーネルごとに `ResourceBinding` を再生成すると「グローバル数 × カーネル数」分の重複オブジェクトが生じる。
...
生成コード: `Emit.CodeEmitter` が `Properties`（`Shader.PropertyToID` キャッシュ）、コンストラクタ、カーネルごとの `readonly struct`（`NumThreadsX/Y/Z` 定数、`Set_<Name>` リソース setter、`DispatchThreads`/`DispatchGroups`）、外側レベルの plain global／cbuffer メンバー setter（`Emit.HlslTypeMap` で HLSL 型→C# 型／`ComputeShader.Set*` メソッドへ変換）、`StructuredBuffer<T>` 系列が参照するユーザー struct の `[StructLayout(Sequential)]` 要素構造体を出力する。命名規則は「HLSL 名をそのまま使う」（決定事項どおり）。`int2/3/4` と `uint2/3/4` の uniform は `ComputeShader.SetInts` に成分ごとの引数（`x, y, z, w`。`uintN` は `unchecked((int)x)` で再解釈）を渡す setter として生成される — `ComputeShader` に単一ベクトル/配列の overload が無いため、これらの型を要素に持つ配列や `StructuredBuffer<T>` 要素 struct のフィールドは引き続き `TKH1005` で未対応のまま。
